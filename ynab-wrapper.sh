...
YNAB_HOME="${YNAB_HOME:-$HOME/ynab-amazon-categorizer}"
...
# files modified in the last 2 hours by default — tune via env if needed.
...
# Use find to be portable; -mmin works on macOS and Linux.
candidates=()
...
for f in "${candidates[@]+"${candidates[@]}"}"; do
  type="$(identify_csv "$f")"
  case "$type" in
    items)        [ -z "$latest_items" ]        && latest_items="$f" ;;
    orders)       [ -z "$latest_orders" ]       && latest_orders="$f" ;;
    transactions) [ -z "$latest_transactions" ] && latest_transactions="$f" ;;
  esac
done
...
if [ -f "$ITEMS_CSV" ];  then csv_args+=("--amazon-items-csv"        "$ITEMS_CSV");  fi
if [ -f "$ORDERS_CSV" ]; then csv_args+=("--amazon-orders-csv"       "$ORDERS_CSV"); fi
if [ -f "$TX_CSV" ];     then csv_args+=("--amazon-transactions-csv" "$TX_CSV");     fi
